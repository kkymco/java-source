    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a SuperCoin address (e.g. SuperCoinfwYhBmGXcFP2Po1NpRUEiK8km2)"));
#endif
...
void SendCoinsDialog::on_sendButton_clicked()
{
	...
	if(CoinControlDialog::coinControl->GetAnonymousSend())
	{
		retval = QMessageBox::question(this, tr("Confirm send coins"),
                          tr("Are you sure you want to send %1? Remember with SuperSend 1% fee (minimum 0.5 SUPER) will be added to your transaction. Also your minimum balance should be more than the twice of the amount (plus fee) you send. Otherwise please use regular send.").arg(formatted.join(tr(" and "))),
	...
	if(CoinControlDialog::coinControl->GetAnonymousSend())
	{
		...
		if(totalSend > MAX_ALLOWED_ANONYMOUS_SEND)
		{
			QMessageBox::warning(this, tr("Send Coins"),
				tr("SuperSend only allows maximum of 100 SUPERs now. Please reduce send amount or use regular send."),
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid SuperCoin address"));
        }
        else
        {
